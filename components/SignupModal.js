      ...
      <div className="bg-white/60 backdrop-blur-lg rounded-3xl shadow-2xl p-8 w-full max-w-md relative border border-white/40 flex flex-col items-center" style={{boxShadow: '0 8px 32px 0 rgba(251,146,60,0.25), 0 1.5px 8px 0 rgba(31,38,135,0.10)'}}>
        ...
        <div className="flex flex-col items-center w-full">
          <div className="w-16 h-16 rounded-full bg-orange-100 flex items-center justify-center mb-4 shadow-lg">
            <i className="fa fa-user-plus text-3xl text-orange-500"></i>
          </div>
          <h2 className="text-3xl font-extrabold text-orange-500 mb-2 text-center tracking-tight">Create Account</h2>
          <p className="text-gray-600 mb-6 text-center text-base">Sign up to start ordering from CampusEats</p>
        </div>
        <form className="flex flex-col gap-4 w-full">
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-orange-400"><i className="fa fa-user"></i></span>
            <input type="text" placeholder="Name" className="pl-10 pr-4 py-3 rounded-full border border-orange-200 bg-white/80 backdrop-blur text-gray-800 w-full focus:ring-2 focus:ring-orange-300 focus:outline-none transition" />
          </div>
          <div className="relative">
          ...
          <Button className="bg-orange-500 text-white py-3 rounded-full font-bold text-lg mt-2 shadow-lg cursor-pointer transition-all hover:bg-orange-600 hover:scale-105">Sign Up</Button>
        </form>
        <p className="mt-6 text-center text-sm text-gray-700">Already have an account? <a href="#" className="text-orange-500 font-semibold hover:underline" onClick={e => { e.preventDefault(); onClose(); onSwitchToLogin(); }}>Login</a></p>
      </div>
    </div>
  );
};
